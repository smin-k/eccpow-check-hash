...
func MakeDecision(header *types.Header, colInRow [][]int, outputWord []int) bool {
	fmt.Printf("\n______make_decision_______\n", )
	//fmt.Printf("_outputWord:_%v_____\n", outputWord)
	//fmt.Printf("_colInRow:_%v_____\n", colInRow)

	parameters, difficultyLevel := setParameters(header)

	fmt.Printf("parameters:_%v\n", parameters)
	fmt.Printf("difficultyLevel:_%v\n", difficultyLevel)
	fmt.Printf("colInRow:_%v\n", colInRow)
	fmt.Printf("outputWord:_%v\n", outputWord)
	...
	fmt.Printf("!!!con0:_true\n" )
	var numOfOnes int
	for _, val := range outputWord {
		numOfOnes += val
	}
	
	fmt.Printf("!!!!!!!!con1:_%v\n", numOfOnes >= Table[difficultyLevel].decisionFrom)
